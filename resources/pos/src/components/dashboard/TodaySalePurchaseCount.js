...
import { fetchTotalOrder } from "../../store/action/ordersAction";
...
const TodaySalePurchaseCount = (props) => {
    ...
    useEffect(async() => {
        todaySalePurchaseCount();
        fetchAllSalePurchaseCount();
        setTotalOrder(await fetchTotalOrder())
    }, []);
        ...
        <Row className="g-4">
            <Col className="col-12 mb-4">
                <Row>
                    ...
                    />
                    <Widget
                        title={getFormattedMessage("orders.title")}
                        // onClick={() => onClick("app/orders", "manage_sale")}
                        allConfigData={allConfigData}
                        className={`bg-primary ${
                            config &&
                            config.filter((item) => item === "manage_sale")
